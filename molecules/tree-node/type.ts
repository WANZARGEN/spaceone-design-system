...
export interface TreeNodeStateType<T=any, S extends NodeState = NodeState> {
    level: number;
    padSize: string;
    toggleSize: string;
...
type ClassNamesType<T=any, S extends NodeState = NodeState> = (node: TreeItem<T, S>) => {[name: string]: boolean};

export interface TreeNodeProps<T=any, S extends NodeState = NodeState> extends TreeNodeStateType, TreeNode<T, S> {}

export interface TreeNode<T=any, S extends NodeState = NodeState> {
    data: T;
...
export interface InitTreeNode<T, S extends NodeState = NodeState> {
    data?: T;
    children?: TreeNodeProps<T, S>[] | boolean;
    state?: S;
}

export const getDefaultNode = <T=any, S extends NodeState = NodeState>(data: T, init?: InitTreeNode<T, S>): TreeNode<T, S> => ({
    data,
    children: false,
...
export const getTreeItem = <T=any, S extends NodeState = NodeState>(
...
export interface TreeItem<T=any, S extends NodeState = NodeState> {
    key: number;
    level: number;
...
export class TreeNodeState<
    data=any, state extends NodeState = NodeState,
...
export interface TreeNodeMetaState<T=any, S extends NodeState = NodeState> {
    nodes: TreeNode<T, S>[];
    selectedNodes: TreeItem<T, S>[];
    firstSelectedNode: TreeItem<T, S>;
}

export type TreeNodeEventListener<T=any, S extends NodeState = NodeState>
    = (node: TreeItem<T, S>, matched: TreeItem<T, S>[], e: MouseEvent) => void;

export interface TreeNodeEventListeners<T=any, S extends NodeState = NodeState> {
